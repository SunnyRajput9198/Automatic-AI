...
class FileManager:
    ...
    def _safe_path(self, filename: str, workspace: str = "shared") -> Path:
        """Prevent path traversal attacks"""

        if workspace == "shared":
            base = self.shared_workspace
        else:
            base = self.base_dir / workspace
    ...
    def read_file(self, filename: str, workspace: str = "shared") -> Optional[str]:
        """Read file content"""
        try:
            filepath = self._safe_path(filename, workspace)
        except ValueError as e:
            logger.error("file_manager_invalid_path", error=str(e))
            return None
        ...
        try:
            ...
            logger.info("file_manager_read", filename=filename, size=len(content))
            return content
    ...
    def write_file(
        self, filename: str, content: str, workspace: str = "shared"
    ) -> bool:
        ...
        try:
            ...
            logger.info("file_manager_written", filename=filename, size=len(content))
            return True
        except Exception as e:
    ...
    def append_file(
        ...
        try:
            with open(filepath, "a") as f:
                f.write(content)
            logger.info("file_manager_appended", filename=filename, size=len(content))
            return True
        except Exception as e:
