...
class Week3Evaluator:
    ...
    async def run_learning_experiment(
        ...
        """
        logger.info(
            "learning_experiment_starting", task=task_description, runs=num_runs
        )
        ...
        for run_num in range(1, num_runs + 1):
            ...
            with get_db_context() as db:
                task = Task(
                    user_input=task_description, status="PENDING", id=str(uuid.uuid4())
                )
                db.add(task)
                db.commit()
                task_id = task.id
            ...
            with get_db_context() as db:
                ...
                if task:
                    ...
                    completed_task = next(
                        (
                            t
                            for t in global_cost_tracker.completed_tasks
        ...
        if len(results["runs"]) >= 2:
            ...
            results["improvement"] = {
                "duration_reduction_sec": round(
                    first_run["duration_sec"] - last_run["duration_sec"], 2
                ),
                ...
                "retry_reduction": max(
                    0,
                    first_run.get("total_retries", 0)
                    - last_run.get("total_retries", 0),
                ),
                "search_reduction": max(
                    0,
                    first_run.get("search_operations", 0)
                    - last_run.get("search_operations", 0),
                ),
                "llm_call_reduction": max(
                    0, first_run.get("llm_calls", 0) - last_run.get("llm_calls", 0)
                ),
            }
    ...
    def _print_learning_summary(self, results: Dict[str, Any]):
        ...
        if "improvement" in results:
            ...
            print(
                f"   Duration:    {imp['duration_reduction_pct']:+.1f}% ({imp['duration_reduction_sec']:+.1f}s)"
            )
    ...
    async def quick_demo(self, orchestrator_func=None):
        ...
        for task in test_tasks:
            ...
            await self.run_learning_experiment(
                task_description=task, num_runs=2, orchestrator_func=orchestrator_func
            )
